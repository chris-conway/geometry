...
import jts.operation.buffer.validate.BufferResultValidator;
import jts.util.StringUtil;
    ...
    if (buffer == null) {
      ...
      if (getBuffer().getClass() == GeometryCollection.class && getBuffer().isEmpty()) {
        try {
          ...
          jts.util.Assert.shouldNeverReachHere();
        }
      }
    }
    return buffer;
  ...
  private void addContainsTest() {
    addTest(new Test("Contains Test") {
      public void test() throws Exception {
        ...
        jts.util.Assert.isTrue(getOriginal().isValid());
        if (bufferDistance > 0) {
          Assert.assertTrue(
